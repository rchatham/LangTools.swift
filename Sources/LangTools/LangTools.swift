...
public protocol LangTools {
    ...
    func stream<Request: LangToolsStreamableRequest>(request: Request) throws -> AsyncThrowingStream<Request.Response, Error>
...
extension LangTools {
    ...
    public func stream<Request: LangToolsStreamableRequest>(request: Request) throws -> AsyncThrowingStream<Request.Response, Error> {
            ...
            Task {
                ...
                do {
                    ...
                    if let completionRequest = try completionRequest(request: request, response: try request.update(response: combinedResponse)) {
                        for try await response in try stream(request: completionRequest) {
                            continuation.yield(try request.update(response: response))
                        }
                    }
                } catch {
    ...
    private func stream<Request: LangToolsStreamableRequest>(request: Request) -> AsyncThrowingStream<any LangToolsStreamableResponse, Error> {
        return AsyncThrowingStream { cont in
            Task {
                for try await response in try stream(request: request) {
                    cont.yield(response)
                }
                cont.finish()
            }
