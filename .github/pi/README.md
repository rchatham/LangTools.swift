...
When upgrading Pi, update all workflow version pins, `.github/pi/runtime/package.json`, and its lockfile, then verify the downloaded npm tarball SHA-256 before changing the recorded checksum. Runtime dependencies are installed with `npm ci --ignore-scripts` from the committed integrity-bearing lockfile before the verified top-level package is extracted.
