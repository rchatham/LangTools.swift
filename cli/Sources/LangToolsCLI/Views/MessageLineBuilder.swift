...
enum MessageLineBuilder {
    ...
    /// Body indent for a tool message, derived from the leading spaces of its
    /// header `prefix` so the body aligns directly under its own header without
    /// redundant extra leading spaces.
    ///
    /// - Call: header `↳ Call` (no leading spaces) -> body at column 0.
    /// - Result: header `  ✓ Result` (two leading spaces, the call→result
    ///   nesting) -> body at column 2.
    static func bodyIndent(forPrefix prefix: String) -> String {
