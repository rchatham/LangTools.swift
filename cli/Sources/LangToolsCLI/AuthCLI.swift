...
private enum AuthSubcommand {
    ...
    case exportSession(AuthProvider, OutputFormat)
    case status(AuthProvider, OutputFormat)
    case logout(AuthProvider)
    ...
    init(arguments: [String]) throws {
        ...
        switch command {
        case "login":
            self = .login(try AuthProvider(arguments: remaining))
        case "export-session":
            self = .exportSession(try AuthProvider(arguments: remaining), try OutputFormat(arguments: remaining))
        case "status":
            self = .status(try AuthProvider(arguments: remaining), try OutputFormat(arguments: remaining))
        case "logout":
            self = .logout(try AuthProvider(arguments: remaining))
        default:
            throw CLIError.usage(AuthCLI.usage)
        }
    }
...
private enum AuthProvider: String {
    ...
    init(arguments: [String]) throws {
        guard let first = arguments.first, let provider = AuthProvider(rawValue: first.lowercased()) else {
            throw CLIError.usage(AuthCLI.usage)
        }
        self = provider
    }
...
private struct AuthCommands {
    ...
    func login(provider: AuthProvider) async throws {
    ...
    func exportSession(provider: AuthProvider, format: OutputFormat) throws {
        guard provider == .openai else { throw CLIError.unsupportedProvider }
    ...
    func status(provider: AuthProvider, format: OutputFormat) throws {
        guard provider == .openai else { throw CLIError.unsupportedProvider }
        let session = try? store.load()
        switch format {
        case .json:
    ...
    func logout(provider: AuthProvider) throws {
        guard provider == .openai else { throw CLIError.unsupportedProvider }
        try store.remove()
