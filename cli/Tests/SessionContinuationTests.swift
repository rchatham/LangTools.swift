...
final class SessionContinuationTests: XCTestCase {
    ...
    func testListAndPrefixLoadAreScopedToExactWorkingDirectory() throws {
        try withManager { manager, root in
            ...
            let current = manager.createSession(name: "here", workingDirectory: repo, model: "model")
            _ = manager.createSession(name: "elsewhere", workingDirectory: other, model: "model")
            ...
            _ = manager.createSession(name: "also-here", workingDirectory: repo, model: "model")
    ...
    func testSnapshotReplacesInsteadOfDuplicatingAndSurvivesReload() throws {
        try withManager { manager, root in
            let session = manager.createSession(name: "continue", workingDirectory: root.path, model: "model")
