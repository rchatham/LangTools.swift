...
class AnthropicTests: XCTestCase {
    ...
    func testChatStream() async throws {
        ...
        for try await response in try api.stream(request: Anthropic.MessageRequest(model: .claude35Sonnet_20240620, messages: messages)) {
    ...
    func testChatStreamResponse() async throws {
        ...
        var results: [Anthropic.MessageResponse] = []
        for try await response in try api.stream(request: Anthropic.MessageRequest(model: .claude35Sonnet_20240620, messages: [.init(role: .user, content: "Hi")], stream: true)) {
            results.append(response)
        }
